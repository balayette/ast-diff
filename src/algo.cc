...
void addPairOfIsomorphicDescendants(GumtreeData &data, Tree &t1, Tree &t2) {
  ...
  foreach_pair(desc1.begin(), desc1.end(), desc2.begin(), desc2.end(),
               [&](Tree &a, Tree &b) {
                 std::cout << a.GetValue() << " iso " << b.GetValue() << '?';
                 if (a.IsIsomorphic(b)) {
...
void isomorphicPair(GumtreeData &data, Tree &t1, Tree &t2) {
  auto iso1 =
      FindAll(data.DestTree, [&](Tree &tx) { return t1.IsIsomorphic(tx); });
  auto iso2 =
      FindAll(data.SourceTree, [&](Tree &tx) { return tx.IsIsomorphic(t2); });
...
void BottomUp(Tree &T1, Tree &T2, Mappings &M) {
  ...
  if (!M.ContainsMappingFirst(T1) &&
      std::any_of(children.begin(), children.end(), [&](Tree::ptr &child) {
